...
def flatten(
    ...
    def _flatten(object_, key):
        ...
        # check if object (value in dictionary) is iterable
        elif isinstance(object_, dict): # returns True if object_ (value) is dictionary
            for object_key in object_: # for every key in dictionary
                if not (not key and object_key in root_keys_to_ignore): 
                    _flatten(
                        object_[object_key], # first input is value 
                        _construct_key( # second input is the output of _construct_key function above
                            key,
                            separator,
                            object_key,
                            replace_separators=replace_separators))
        elif isinstance(object_, (list, set, tuple)):
            for list_item in object_:
                for object_key in list_item: # for every key in dictionary
                    if not (not key and object_key in root_keys_to_ignore): 
                        _flatten(
                            list_item[object_key], # first input is value 
                            _construct_key( # second input is the output of _construct_key function above
                                key,
                                separator,
                                object_key,
                                replace_separators=replace_separators))
            #x = [_flatten(object_) for d in object_]
